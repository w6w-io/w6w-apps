...
methods. Sixty-six apps add a fourth question — **is this tenant's own host reachable?** —
as a `kind: "dependency"` check, because "the site is gone" and "the token expired" are
different problems with different fixes.

Across the pack that comes to **1,008 checks**: 394 live probes, 249 declared absences, and 367
...
| [adalo](apps/adalo/README.md) | [custom Vercel status page](https://status.adalo.com/api/v2/components.json) — not Statuspage/Instatus-shaped; `/summary.json` is page-level only, `/components.json` names a real "Collections API" component | yes | `GET /v0/apps/{appId}/collections/{probe}` | no | `service` · ~~quota~~ · 1 derived |
| [affinity](apps/affinity/README.md) | [Statuspage](https://status.affinity.co/api/v2/summary.json) — verified against the `affinity.statuspage.io` decoy (different page id, generic components, frozen since 2023) | yes | `GET /auth/whoami` — documented exempt from the monthly call quota it's checked alongside | yes | `service` · `quota` · 1 derived |
| [agencyzoom](apps/agencyzoom/README.md) | none published | no | `POST /v1/api/auth/login` | no — no rate-limit header of any kind found live | ~~service~~ · ~~quota~~ · 1 derived |
...
| [crunchbase](apps/crunchbase/README.md) | none — `status.crunchbase.com` fails DNS entirely, `crunchbase.statuspage.io` is an unclaimed decoy that 302s to statuspage.io's own marketing page | no | `GET /data/autocompletes?query=crunchbase&limit=1` — the one endpoint every license tier includes, even Basic | no — no rate-limit header on any response, only a prose-documented 200/min ceiling | ~~service~~ · ~~quota~~ · 1 derived |
...
| [dust](apps/dust/README.md) | [Statuspage](https://status.dust.tt/api/v2/summary.json) — covers `API`/`Conversations`/`Data Sources` plus both regions; drops the deprecated `Dust App Platform` and unrelated connector/product components | yes | `GET /w/{workspaceId}/...` (probe path) | no (declared unavailable) | `service` · ~~quota~~ · 1 derived |
...
| [formidableforms](apps/formidableforms/README.md) | none published — self-hosted plugin, no central vendor status | no | `GET /frm/v3/forms` | no — the only ceiling is the customer's own web host | ~~service~~ · ~~quota~~ · `site` · 1 derived |
...
| [gamma](apps/gamma/README.md) | [Instatus](https://status.gamma.app/history.atom) — checked against the abandoned `gammaapp.statuspage.io` decoy (302s to statuspage.io's own marketing page) | yes | `GET /themes` | no | `service` · 1 derived |
...
| [givebutter](apps/givebutter/README.md) | [Statuspage](https://givebutter.statuspage.io/api/v2/summary.json) | yes | `GET /v1/campaigns?per_page=1` | yes (`x-ratelimit-limit`/`x-ratelimit-remaining` headers, measured 200 not the documented 500) | `service` · `quota` · 1 derived |
...
| [google-admin](apps/google-admin/README.md) | [JSON](https://www.google.com/appsstatus/dashboard/incidents.json), filtered to the "Admin Console" service (shared backend with the Directory API) | yes | `oauth2`: `GET /admin/directory/v1/users?customer=my_customer&maxResults=1` · `service-account`: `POST https://oauth2.googleapis.com/token` (JWT grant) | no | `service` · ~~quota~~ · 2 derived |
...
| [googlesearchconsole](apps/googlesearchconsole/README.md) | none published — checked all three Google incident feeds this pack's other `google-*` apps use (Workspace dashboard, Ads/Analytics dashboard, Cloud incidents.json); none names Search Console | no | `GET /webmasters/v3/sites` | no — Google's own docs say usage is read from the Cloud Console UI, not any field/header this API exposes | ~~service~~ · ~~quota~~ · 1 derived |
...
| [goto-webinar](apps/goto-webinar/README.md) | [Statuspage](https://status.goto.com/api/v2/summary.json), tracking the "GoTo Webinar API" component among ~70 unrelated GoTo products sharing one page | yes | `GET /identity/v1/Users/me` | no | `service` · ~~quota~~ · 1 derived |
...
| [hackernews](apps/hackernews/README.md) | none published — `hacker-news.firebaseio.com` is a read replica of HN's internal data, not a separately-operated product with a status surface of its own | no | none (fully public, no credential) | no — the README states explicitly "there is currently no rate limit" | `service` (real `GET /v0/maxitem.json` reachability probe) · ~~quota~~ · 0 derived |
...
| [heartbeat](apps/heartbeat/README.md) | [Statuspage](https://status.heartbeat.chat/api/v2/summary.json) | yes | `GET /roles` | yes | `service` · `quota` · 1 derived |
...
| [instapaper](apps/instapaper/README.md) | none published — `status.instapaper.com` does not resolve (NXDOMAIN) and `instapaper.statuspage.io` is the unclaimed-Statuspage decoy | no | `POST /api/1/account/verify_credentials` | no | ~~service~~ · 1 derived |
...
| [invoiceninja](apps/invoiceninja/README.md) | [RSS](https://status.invoiceninja.com/rss) (hosted invoicing.co service only — a self-hosted install is judged by its own `instance` check instead) | yes | `GET /api/v1/ping` (hosted); `GET /api/v1/health_check` (per-connection `instance`, SIGNED — both a missing and an invalid token come back 403 identically to `ping`) | no | `service` · `instance` · 1 derived |
...
| [jira-service-management](apps/jira-service-management/README.md) | [Statuspage](https://jira-service-management.status.atlassian.com/api/v2/summary.json) — its own page, distinct from `jira`'s | yes | `GET /rest/servicedeskapi/info` (unauthenticated; `site` also reads `isLicensedForUse`) | no | `service` · ~~quota~~ · `site` · 2 derived |
...
| [judgeme](apps/judgeme/README.md) | [Statuspage](https://status.judge.me/api/v2/summary.json), genuine and claimed (`page.name: "Judge.me"`); reports the worst of its two Judge.me-owned components (Admin, Storefront widgets) and ignores the three unrelated ones (AliExpress importer, Shopify Admin/Storefront) | yes | `GET /settings` | no — no rate-limit/quota signal anywhere, in the OpenAPI doc or on live response headers | `service` · ~~quota~~ · 1 derived |
...
| [justcall](apps/justcall/README.md) | [Statuspage](https://status.justcall.io/api/v2/summary.json) | yes | `GET /v2.1/users?per_page=1` | yes (`X-Rate-Limit-*` hourly + burst headers) | `service` · `quota` · 1 derived |
...
| [kintone](apps/kintone/README.md) | [Statuspage](https://status.kintone.com/api/v2/summary.json) | yes | `GET /k/v1/app.json?id={testAppId}` | no | `service` · `site` · 1 derived |
...
| [knack](apps/knack/README.md) | [Statuspage](https://status.knack.com/api/v2/summary.json) — tracks the `API` component specifically, distinct from `Builder`/`Live App`/the vendor's own marketing and support components on the same page | yes | `GET /objects/{testObject}/records?rows_per_page=1` — no whoami exists, so a per-connection Object key is collected at connect time just to prove liveness | yes — reads `X-PlanLimit-*`/`X-RateLimit-*` headers when present, `unknown` when not (never assumed) | `service` · `quota` · 1 derived |
...
| [lawmatics](apps/lawmatics/README.md) | [Better Stack](https://status.lawmatics.com/index.json) — `lawmatics.statuspage.io` is the unclaimed-Statuspage decoy; scoped to the "Lawmatics OAuth2.0 API" resource specifically, not the page's four unrelated resources (app client, e-sign, website) | yes | `GET /v1/users/me` | no — a documented 50 req/min limit with no header of any kind, only the 429 itself | `service` · ~~quota~~ · 1 derived |
...
| [meistertask](apps/meistertask/README.md) | [Sorry](https://status.meistertask.com/api/v1/status) — not Statuspage/Instatus-shaped; the Statuspage-style paths 404 as the SPA shell, and the real JSON lives at Sorry-specific `/api/v1/*` paths. Reports the page plus the dedicated `API` component (id 1791), taking the worse of the two | yes | `GET /persons/me` | no — a documented 120 req/60s ceiling with a 180s block on excess, but no header of any kind on success or 4xx | `service` · ~~rate-limit~~ · 2 derived |
| [mercury](apps/mercury/README.md) | [incident.io status page](https://status.mercury.com/api/v2/summary.json), rolled up from Support/Bill Pay & Invoicing/Integrations & API/Check Deposits/Mercury Personal components | yes | `GET /categories?limit=1` | no — no rate-limit headers found on any response; declared `informational` | `service` · `quota` · 1 derived |
...
| [onesignal](apps/onesignal/README.md) | [Statuspage](https://status.onesignal.com/api/v2/summary.json) | yes | `GET /apps/{appId}/segments?limit=1` | no — no `X-RateLimit-*` header on success, only a bare `Retry-After` at 429 | `service` · ~~quota~~ · 1 derived |
...
| [ontraport](apps/ontraport/README.md) | [Statuspage](https://ontraport.statuspage.io/api/v2/summary.json) — the obvious `status.ontraport.com` host is a decoy that 302s to Statuspage's own generic marketing page | yes | `GET /1/Contacts/getInfo` | yes — `X-Rate-Limit-*` headers present on every request, even unauthenticated (180 req/min/account) | `service` · `quota` · 1 derived |
...
| [opusclip](apps/opusclip/README.md) | [Instatus](https://status.opus.pro/api/v2/summary.json) — a component's own `status` can read `OPERATIONAL` while it carries an open `MAJOROUTAGE` incident (confirmed live), so this check folds in each component's `activeIncidents` too | yes | `GET /api/social-accounts?q=mine` | no — the monthly cap (900 credits/workspace) and concurrency limit are enforced purely by refusal (403/429), with no endpoint that reads headroom in advance | `service` · ~~quota~~ · 1 derived |
...
| [planningcenter](apps/planningcenter/README.md) | [Statuspage](https://status.planningcenter.com/api/v2/summary.json) — one redirect from the guessable `status.planningcenteronline.com` | yes | `GET /current/v2/me` | yes | `service` · `quota` · 1 derived |
...
| [recharge](apps/recharge/README.md) | [Statuspage](https://status.getrecharge.com/api/v2/summary.json) — tracks the "Recharge API" component only, distinct from the Shopify/BigCommerce platform components the same page also carries | yes | `GET /token_information` — the one endpoint with no declared scope | yes — undocumented `x-recharge-limit: <used>/<cap>` header, confirmed live incrementing across calls | `service` · `quota` · 1 derived |
| [recruitee](apps/recruitee/README.md) | [Statuspage-shaped page](https://status.tellent.com/api/v2/summary.json) at Tellent (Recruitee's parent brand), filtered to its own 6-component group rather than the page-wide indicator | yes | `GET /c/{companyId}/admin` | no — no rate-limit headers found on any response; declared `informational` | `service` · `quota` · 1 derived |
...
| [respondio](apps/respondio/README.md) | real, self-identifying PagerDuty-hosted page (`status.respond.io`) that renders entirely client-side — every Statuspage/RSS/Atom-shaped path, and a made-up one, answers the identical HTML shell while a genuinely nonexistent path 404s; declared `informational` | no | `GET /space/user?limit=1` — no whoami/me endpoint exists in the documented surface, same probe respond.io's own `mcp-server` uses | no (no quota check — no rate-limit header ever observed) | ~~service~~ · 1 derived |
...
| [shippo](apps/shippo/README.md) | [Statuspage](https://status.goshippo.com/api/v2/summary.json) — Shippo's own top-level components (REST API, Web Dashboard, Platform API) decide the verdict; ~65 carriers nested under a "Carrier API" group are named in the message but never roll into it, since a carrier outage isn't a Shippo outage | yes | `GET /carrier_accounts?results=1` | no — per-object-type/verb/environment ceiling (11 × 4 × 2), no header exposed on any response; declared `informational` | `service` · ~~quota~~ · `account` · 1 derived |
...
| [simplybook](apps/simplybook/README.md) | none — `status.simplybook.me`/`.it` 404, `simplybook.statuspage.io` is the unclaimed-decoy pattern (302 to statuspage.io's marketing page) | no | `GET /admin/services` | no (declared unavailable) | ~~service~~ · ~~quota~~ · 1 derived |
...
| [telnyx](apps/telnyx/README.md) | [Statuspage](https://status.telnyx.com/api/v2/summary.json), genuine and claimed (`page.name: "Telnyx"`); two component names ("API V1"/"API V2") repeat with no group field to disambiguate, so the check reads the page-level indicator plus two unambiguous components (Number Lookup API, Outbound Calling Services) | yes | `GET /v2/phone_numbers/slim?page[size]=1` | no — no rate-limit header found on any response | `service` · ~~quota~~ · 1 derived |
...
| [textmagic](apps/textmagic/README.md) | [Statuspage](https://status.textmagic.com/api/v2/summary.json) | yes | `GET /ping` | yes (account credit balance via `GET /user`) | `service` · `quota` · ~~request-rate~~ · 1 derived |
...
| [tickettailor](apps/tickettailor/README.md) | [Statuspage](https://status.tickettailor.com/api/v2/summary.json) | yes | `GET /v1/overview` | yes (`x-rate-limit-*` headers measured live) | `service` · `quota` · 1 derived |
...
| [tiktok-lead-generation](apps/tiktok-lead-generation/README.md) | none — the status page is a client-rendered SPA shell with no JSON/RSS/Atom feed found behind it | no | `GET /oauth2/advertiser/get/` — needs no `advertiser_id`/`business_id` scope a credential might legitimately lack | no — no rate-limit header found on any route | ~~service~~ · ~~quota~~ · 1 derived |
...
| [tumblr](apps/tumblr/README.md) | [RSS feed](https://automatticstatus.com/rss) at Automattic's shared status page — `status.tumblr.com` itself is a decoy blog, not a status page | yes | `GET /v2/user/info` | no | `service` · ~~quota~~ · 1 derived |
...
| [wati](apps/wati/README.md) | [Zoho StatusIQ](https://status.wati.io/rss) — hand-parsed (grouped by title, not `<guid>`, since StatusIQ reuses one guid per publish batch) rather than a declared `feed:` | yes | `GET /api/ext/v3/account/credits` | no (declared unavailable) | `service` · `tenant` · ~~quota~~ · 1 derived |
...
| [webinarjam](apps/webinarjam/README.md) | [Statuspage](https://status.webinarjam.com/api/v2/summary.json) — tracks the `API` component specifically, distinct from the `WebinarJam App`/`EverWebinar App` end-user dashboards on the same page | yes | `GET /webinarjam/webinars` (`api_key` form field, never a header) | no — vendor documents only a flat 20 requests/second prose ceiling (bare 429), no rate-limit header of any kind observed live; declared `informational` | `service` · ~~quota~~ · 1 derived |
...
| [workable](apps/workable/README.md) | [Statuspage](https://workable.statuspage.io/) — genuine `Workable Recruiting` component group, not an unclaimed decoy | yes | `GET /accounts/:subdomain` — signed, because every `*.workable.com` subdomain shares one Cloudflare edge and answers the identical unauthenticated `401` regardless of whether the account exists | no — documented (`X-Rate-Limit-*`, 10 req/10s) but unverifiable live without a real access token | `service` · `quota` · `account` · 1 derived |
...
| [zoho-bookings](apps/zoho-bookings/README.md) | [StatusIQ RSS](https://us.zohostatus.com/rss) — matched on the exact "Zoho Bookings" component | yes | `GET /bookings/v1/json/workspaces` (per-region, ×8 DCs) | no (no rate-limit header published) | `service` · ~~quota~~ · 8 derived |
| [zoho-calendar](apps/zoho-calendar/README.md) | [StatusIQ RSS](https://us.zohostatus.com/rss) — matched on the exact "Zoho Calendar" component | yes | `GET /api/v1/calendars` (per-region, ×8 DCs) | no (no rate-limit header published) | `service` · ~~quota~~ · 8 derived |
...
| [zoho-recruit](apps/zoho-recruit/README.md) | [StatusIQ RSS](https://us.zohostatus.com/rss) — matched on the exact "Zoho Recruit" component | yes | `GET /recruit/v2/Candidates` (per-region, ×10 DCs) | no (no rate-limit header published) | `service` · ~~quota~~ · 10 derived |
| [zoho-sheet](apps/zoho-sheet/README.md) | [StatusIQ RSS](https://us.zohostatus.com/rss) — matched on the exact "Zoho Sheet" component | yes | `POST /api/v2/workbooks` (per-region, ×7 DCs) | no (no rate-limit header published) | `service` · ~~quota~~ · 7 derived |
...
| [zuora](apps/zuora/README.md) | [Statuspage](https://zuora.statuspage.io/) — per-region/cloud component groups (e.g. `Production API` under NA1 vs NA2), resolved to the connection's own region so another region's outage doesn't turn this connection red | yes | `GET /object-query/accounts?pageSize=1` | yes — `ratelimit-*` headers on every response, tenant-level (per-minute window reported) | `service` · `quota` · 1 derived |
