...
/**
 * `crm.contact.list` — verified against
 * `api-reference/crm/contacts/crm-contact-list.html`. DEPRECATED by the
 * vendor in favor of `crm.item.list` (see `README.md`).
 *
...
const action: ActionDefinition<Input, ListOutput> = {
  ...
  resource: "contact",
  title: "List Contacts",
  description: "Search contacts with an optional filter, field selection and sort.",
  params: LIST_PARAMS,
  output: [
    { key: "results", label: "Contacts", type: "array" },
  ...
  async execute(input, ctx) {
    ...
    ctx.log("info", "listing Bitrix24 contacts", { start: input.start ?? 0 });
