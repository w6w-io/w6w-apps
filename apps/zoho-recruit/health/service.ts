/**
 * Is Zoho Recruit up? — Zoho's StatusIQ (Site24x7) status page.
 *
 * Verified live 2026-09-05: `https://us.zohostatus.com/rss` is a Site24x7
 * StatusIQ page that lists every Zoho product — Mail, CRM, Bigin, Recruit,
 * Books, Desk, ~100 more — as one RSS item per component, titled
 * `"{component} - {status}"`. Fetched live: the entry is exactly
 * `"Zoho Recruit - Operational"`, distinct from the generic Zoho umbrella
 * entries also on the same page. This is the same feed this pack's `zoho`
 * (Zoho CRM), `zohobooks` and `zohodesk` apps already document.
 *
 * Annotation, and why each axis is what it is:
 *   - `kind: "service"` — a different question from "is this credential live"
 *     (the ten derived `auth:oauth2-<region>` checks) or "is there quota
 ...
 *     this hook only finds the "Zoho Recruit" component and reads its status
...
/** Exact component name on the status page — "Zoho Recruit". */
const COMPONENT = "Zoho Recruit";

const service: HealthCheckDefinition = {
  key: "service",
  title: "Zoho Recruit platform status",
  description:
    'Reads the "Zoho Recruit" component off Zoho\'s StatusIQ RSS feed (us.zohostatus.com/rss). ' +
