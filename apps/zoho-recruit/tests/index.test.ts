...
/** Creates start a new resource on every call — retrying a dropped response would duplicate it. */
Deno.test("index: the three creates are not marked idempotent", () => {
  for (const key of ["candidate-create", "job-opening-create", "client-create", "note-create"]) {
    assertEquals(app.actions.find((a) => a.key === key)?.idempotent, false, key);
  }
});

/** Updates, deletes and status changes converge on the same end state no matter how many times they run. */
Deno.test("index: updates, deletes and status changes are marked idempotent", () => {
  for (
    const key of [
      "candidate-update",
      "candidate-delete",
 ...
 * else. An action that hard-coded a `recruit.zoho.<tld>` host — or accepted
...
Deno.test("index: no action hard-codes a host or an absolute URL", async () => {
  for (const a of app.actions) {
    ...
    assert(
      !/zoho\.com|zoho\.eu|zoho\.in|zohocloud|zoho\.sa|zoho\.sg|zoho\.ae|zoho\.jp/.test(src),
...
Deno.test("index: the manifest allows every region's API host and none of the accounts hosts", async () => {
  ...
  assertEquals(manifest.w6w.id, "io.w6w.zoho-recruit");
  for (const region of REGIONS) {
...
Deno.test("index: the icon is byte-identical to apps/zoho's — the same vendor, same mark", async () => {
  const svg = await Deno.readTextFile(new URL("../assets/icon.svg", import.meta.url));
  const zohoSvg = await Deno.readTextFile(
    new URL("../../zoho/assets/icon.svg", import.meta.url),
  );
  assertEquals(svg, zohoSvg);
});
