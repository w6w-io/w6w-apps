...
/**
 * Creating/appending starts a new resource or grows a worksheet on every
 * call — retrying a dropped response would create a duplicate workbook/sheet
 * or append the same rows twice, not converge on the same state.
 */
Deno.test("index: creates/appends/CSV-writes are not marked idempotent", () => {
  for (const key of ["workbook-create", "sheet-create", "sheet-append", "sheet-write-csv"]) {
    assertEquals(app.actions.find((a) => a.key === key)?.idempotent, false, key);
  }
});

/** The converse: a delete and a content-clear converge on the same end state no matter how many times they run. */
Deno.test("index: delete and clear are marked idempotent", () => {
  for (const key of ["sheet-delete", "sheet-clear"]) {
 ...
 * else. An action that hard-coded a `sheet.zoho.<tld>` host — or accepted one
 * as a param — could be pointed somewhere the manifest never allowlisted.
 */
...
Deno.test("index: the manifest allows every region's API host and none of the accounts hosts", async () => {
  ...
  assertEquals(manifest.w6w.id, "io.w6w.zoho-sheet");
  for (const region of REGIONS) {
...
/**
 * The icon is reused byte-for-byte from `apps/zoho/assets/icon.svg` per this
 * app's build instructions (same vendor, same mark) — so it still carries
 * that sibling app's "Zoho CRM" aria-label/title rather than a Sheet-specific
 * one. This only asserts it's a real, non-empty SVG.
 */
Deno.test("index: the icon is a real SVG carrying the Zoho mark", async () => {
  const svg = await Deno.readTextFile(new URL("../assets/icon.svg", import.meta.url));
  assert(svg.startsWith("<svg"), "not an SVG");
  assert(svg.includes("Zoho"), "missing the Zoho aria-label/title");
});
