/**
 * Is Zoho Sheet up? — Zoho's StatusIQ (Site24x7) status page.
 *
 ...
 * Sheet, ~100 more — as one RSS item per component, titled
 * `"{component} - {status}"`. Fetched live: the entry is exactly `"Zoho
 * Sheet - Operational"`, distinct from the generic Zoho umbrella entries and
 * from `"Zoho Docs"`/other Zoho Office Suite components also on the same
 * page. This is the same feed this pack's `zoho` (Zoho CRM), `zohobooks` and
 * `zohodesk` apps already document.
 *
 ...
 *     this hook only finds the "Zoho Sheet" component and reads its status
...
/** Exact component name on the status page — "Zoho Sheet". */
const COMPONENT = "Zoho Sheet";

const service: HealthCheckDefinition = {
  key: "service",
  title: "Zoho Sheet platform status",
  description:
    'Reads the "Zoho Sheet" component off Zoho\'s StatusIQ RSS feed (us.zohostatus.com/rss). ' +
