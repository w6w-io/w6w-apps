...
/** The four creates start a new record on every call; retrying one would create a duplicate. */
Deno.test("index: no create action is marked idempotent", () => {
  for (const key of ["contact-create", "job-create", "task-create", "activity-create"]) {
    assertEquals(app.actions.find((a) => a.key === key)?.idempotent, false, key);
  }
});

/** The four updates and two deactivations converge to the same state on repetition. */
Deno.test("index: every update/delete action is marked idempotent", () => {
  for (
    const key of ["contact-update", "contact-delete", "job-update", "job-delete"]
  ) {
...
Deno.test("index: no action hard-codes a host", async () => {
  for (const a of app.actions) {
    const src = await actionSource(a.key);
    assert(!/jobnimbus\.com/.test(src), `${a.key}: contains a JobNimbus host literal`);
...
Deno.test("index: the auth probe is /contacts, the app's most basic CRM object", async () => {
  const src = code(await Deno.readTextFile(new URL("../auth/bearer-token.ts", import.meta.url)));
  assert(src.includes('"/contacts"'), "auth probe no longer hits /contacts");
});
...
Deno.test("index: the manifest allows the API host and not the status host", async () => {
  ...
  assertEquals(manifest.w6w.id, "io.w6w.jobnimbus");
  assert(manifest.w6w.network.allow.includes("app.jobnimbus.com"));
  // The status host belongs to the health check's own allowlist, not the app's.
  assert(!manifest.w6w.network.allow.includes("status.jobnimbus.com"));
  assertEquals(manifest.w6w.appearance.icon.svg, "./assets/icon.svg");
});

Deno.test("index: the icon wraps the vendor's own favicon", async () => {
  const svg = await Deno.readTextFile(new URL("../assets/icon.svg", import.meta.url));
  // Downloaded verbatim from app.jobnimbus.com/images/favicon.png on
  // 2026-09-01 — 512x512 PNG, 2,152 bytes — wrapped in an <svg><image> per
  // the pack's existing precedent for a vendor that publishes no SVG mark
  // (dialpad, apollo, blandai, gorgias, kustomer).
  assert(svg.startsWith('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"'));
  assert(svg.includes('href="data:image/png;base64,'));
});
