...
Deno.test("create-folder: POSTs to the parent's children collection", async () => {
  ...
  assertEquals(new URL(calls[0].url).pathname, "/v1.0/sites/root/drive/items/P1/children");
  assertEquals(calls[0].method, "POST");
});

Deno.test("create-folder: no parent addressed means the library root", async () => {
...
Deno.test("create-folder: conflictBehavior rides in the body", async () => {
  const { ctx, calls } = mockCtx([{ status: 201, body: {} }]);
