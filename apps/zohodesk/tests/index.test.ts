...
/**
 * Creating a record or attaching a file starts a new resource/side-effect on
 * every call — retrying a dropped response would create a duplicate ticket,
 * a duplicate comment, or a duplicate attachment, not converge on the same
 * state.
 */
Deno.test("index: creates and the comment/attachment actions are not marked idempotent", () => {
  for (
    const key of [
      "ticket-create",
      "contact-create",
...
/** The converse: updates and the bulk-delete/moveToTrash calls converge on the same end state. */
 ...
 * else. An action that hard-coded a `desk.zoho.<tld>` host — or accepted one
...
Deno.test("index: no action hard-codes a host or an absolute URL", async () => {
  for (const a of app.actions) {
    ...
    assert(
      !/zoho\.com|zoho\.eu|zoho\.in|zohocloud|zoho\.sa|zoho\.sg|zoho\.ae/.test(src),
...
Deno.test("index: the manifest allows every region's API host and none of the accounts hosts", async () => {
  ...
  assertEquals(manifest.w6w.id, "io.w6w.zohodesk");
  for (const region of REGIONS) {
...
/**
 * Unlike zohobooks (which declares its quota check unavailable), this app's
 * `quota` check has a real probe — so nothing here is declared unavailable.
 * The test still exists to record that fact rather than assume it silently.
 */
Deno.test("index: no health check is declared unavailable — Zoho Desk publishes a real quota header", () => {
...
Deno.test("index: the icon is the placed vendor mark", async () => {
  ...
  assert(svg.includes("Zoho Desk"), "missing the Zoho Desk aria-label/title");
});
