/**
 * Is Zoho Desk up? — Zoho's StatusIQ (Site24x7) status page.
 *
 * Verified 2026-08-25: `https://us.zohostatus.com/rss` is a Site24x7 StatusIQ
 * page that lists every Zoho product — Mail, CRM, Bigin, Books, Desk, ~100
 * more — as one RSS item per component, titled `"{component} - {status}"`.
 * Fetched live: the entry is exactly `"Zoho Desk - Operational"`, distinct
 * from the generic "Zoho" umbrella entries and from "Zoho CRM"/"Zoho Books"
 * also on the same page. This is the same feed this pack's `zoho` (Zoho CRM),
 * `zohobooks` and `zohomail` apps already document.
 *
 ...
 *     this hook only finds the "Zoho Desk" component and reads its status
...
/** Exact component name on the status page — "Zoho Desk". */
const COMPONENT = "Zoho Desk";

const service: HealthCheckDefinition = {
  key: "service",
  title: "Zoho Desk platform status",
  description:
    'Reads the "Zoho Desk" component off Zoho\'s StatusIQ RSS feed (us.zohostatus.com/rss). ' +
