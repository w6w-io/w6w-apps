...
/** Anything that duplicates on a retry says so; PUTs that replace absolute state don't. */
...
Deno.test("index: declares both health checks", () => {
  ...
  assertEquals(keys, ["service", "site"]);
});

Deno.test("index: no action exposes a `list apps` operation (API Token cannot authenticate Get Apps)", () => {
  const keys = app.actions.map((a) => a.key);
  assert(!keys.includes("apps-list"), "Get Apps requires Password/Session/OAuth2, not API Token");
});

Deno.test("manifest: reverse-DNS id, 1-3 categories, wide network allow, real icon asset", async () => {
  assertEquals(manifest.w6w.id, "io.w6w.kintone");
  assertEquals(manifest.w6w.displayName, "Kintone");
