...
import action from "../../actions/resubscribe.ts";

Deno.test("resubscribe: POSTs id to /users/resubscribe", async () => {
  const { ctx, calls } = mockCtx([{ status: 200, body: { status: 200, message: "Success." } }]);
  const result = await action.execute!({ id: "u1" }, ctx);
  assertEquals(calls.length, 1);
  assertEquals(calls[0].url, "https://api.getvero.com/api/v2/users/resubscribe");
  assertEquals(calls[0].method, "POST");
  assertEquals(JSON.parse(calls[0].body!), { id: "u1" });
  assertEquals(result, { success: true, message: "Success." });
});

Deno.test("resubscribe: rejects a blank id", async () => {
...
Deno.test("resubscribe: a non-2xx response propagates as an Error", async () => {
