...
/**
 * Creating something, sending mail, or moving state forward starts a new
 * resource/side-effect on every call — retrying a dropped response could
 * create a duplicate list/campaign/field or resend a confirmation/campaign
 * email, not converge on the same state.
 */
Deno.test("index: creates, subscribes and sends are not marked idempotent", () => {
  for (
    const key of [
      "list-create",
      "contact-add-bulk",
...
/**
 * The converse: updates, deletes and the unsubscribe/do-not-mail
 * transitions converge on the same end state no matter how many times they
 * run, so the runtime is free to retry them.
 */
 ...
 * else. An action that hard-coded a `campaigns.zoho.<tld>` host — or
 * accepted one as a param — could be pointed somewhere the manifest never
 * allowlisted.
 */
...
Deno.test("index: the manifest allows every region's API host and none of the accounts hosts", async () => {
  ...
  assertEquals(manifest.w6w.id, "io.w6w.zoho-campaigns");
  for (const region of REGIONS) {
...
Deno.test("index: the icon is the placed vendor mark, a non-trivial SVG naming this product", async () => {
  const svg = await Deno.readTextFile(new URL("../assets/icon.svg", import.meta.url));
  assert(svg.trimStart().startsWith("<svg"), "icon.svg does not start with <svg");
  assert(svg.includes("Zoho Campaigns"), "icon.svg does not name Zoho Campaigns");
  assert(svg.length > 500, "icon.svg looks too small to be a real mark");
});
