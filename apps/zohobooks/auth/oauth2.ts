...
import { REGIONS, type ZohoBooksRegion } from "../lib/regions.ts";

/**
 * OAuth 2.0 (`oauth2`) — Zoho Books' only connect path. Register a Zoho API
 ...
 * across all eight.
 *
 * Zoho specifics, verified 2026-08-24 against
 ...
 *   - Scopes are `ZohoBooks.contacts.ALL`, `ZohoBooks.settings.ALL` (covers
 *     Items — Zoho files Items under the general "settings" scope family
 *     rather than a dedicated `items` scope, easy to miss — and also covers
 *     `GET /organizations`, documented as needing `ZohoBooks.settings.READ`),
 *     `ZohoBooks.invoices.ALL` and `ZohoBooks.estimates.ALL` — the union
 *     every action in this app needs. Zoho Books' scope vocabulary is
 *     per-resource with `.ALL` / `.READ` / `.CREATE` / `.UPDATE` / `.DELETE`
 *     suffixes, the same shape as Zoho Mail's.
 ...
 *     `zoho` (Zoho CRM) app. Since this method already fixes one region's
 *     `apiHost` at build time, `api_domain` is only used as a sanity check;
 *     the region's own host is what `lib/client.ts` actually addresses.
 */
function buildOAuth2(region: ZohoBooksRegion): AuthDefinition {
  ...
  return {
    ...
    description:
      `Sign in with Zoho. Use this method only if your Zoho Books organization was created in the ` +
     ...
     * needing only `ZohoBooks.settings.READ` and (unlike every other
     ...
     * (`https://www.zohoapis.com/books/v3/organizations` with no
    ...
    async test({ credential }, ctx) {
      ...
      if (body?.code === 57) {
        return {
          ok: false,
          message: "Zoho Books rejected the access token (code 57). Reconnect this connection.",
        };
      }
      if (body?.code === 14) {
        return {
          ok: false,
          message:
            "Zoho Books received no usable token (code 14) — the credential did not reach the request.",
        };
      }
      return {
        ok: false,
        message:
          `Zoho Books returned HTTP ${res.status}${body?.code ? ` (code ${body.code})` : ""} ` +
          "for /organizations",
      };
    },
