/**
 * Is Zoho Books up? — Zoho's StatusIQ (Site24x7) status page.
 *
 * Verified 2026-08-24: `https://us.zohostatus.com/rss` is a Site24x7 StatusIQ
 * page that lists every Zoho product — Mail, CRM, Bigin, Books, ~100 more —
 * as one RSS item per component, titled `"{component} - {status}"`. Fetched
 * live: the entry is exactly `"Zoho Books - Operational"`, distinct from the
 * generic "Zoho" umbrella entries also on the same page. This is the same
 * feed this pack's `zoho` (Zoho CRM) and `zohomail` apps already document.
 *
 ...
 *     this hook only finds the "Zoho Books" component and reads its status
...
/** Exact component name on the status page — "Zoho Books". */
const COMPONENT = "Zoho Books";

const service: HealthCheckDefinition = {
  key: "service",
  title: "Zoho Books platform status",
  description:
    'Reads the "Zoho Books" component off Zoho\'s StatusIQ RSS feed (us.zohostatus.com/rss). ' +
