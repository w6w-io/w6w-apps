...
import { organizationIdFrom, ZohoBooksClient } from "../lib/client.ts";
 ...
 * wording ("If input json string is empty, mail will be send with default
 * mail content").
 */
const invoiceEmail: ActionDefinition<Input> = {
  ...
  execute(input, ctx) {
    return new ZohoBooksClient(ctx).request(
