...
Deno.test("index: exports 15 actions, one auth method, and two health checks", () => {
...
Deno.test("index: covers list/get for workflow, workflow-run and task", () => {
  const keys = new Set(app.actions.map((a) => a.key));
  assertEquals(keys.has("workflow-list"), true);
  assertEquals(keys.has("workflow-get"), true);
  assertEquals(keys.has("workflow-run-create"), true);
  assertEquals(keys.has("workflow-run-list"), true);
  assertEquals(keys.has("workflow-run-get"), true);
  assertEquals(keys.has("task-list"), true);
  assertEquals(keys.has("task-complete"), true);
});
