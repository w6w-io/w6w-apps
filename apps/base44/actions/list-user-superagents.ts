...
/**
 * `GET /api/v1/monitoring/{workspace_id}/users/{user_id}/superagents` — the
 * Superagent ids one member owns. Ids only, meant to feed
 * `get-superagent-analytics`; `list-superagents` is the workspace-wide
 * inventory with metrics attached.
 */
const action: ActionDefinition = {
  key: "list-user-superagents",
  type: "read",
  resource: "user",
  title: "List User's Superagents",
  description:
    "Superagent IDs owned by one workspace member. Feed each into `get-superagent-analytics`.",
  params: [
    ...
    { key: "active", label: "Messaged in the last 30 days", type: "boolean", advanced: true },
  ],
  output: [
    { key: "superagentIds", type: "array", label: "Superagent IDs" },
  ...
  async execute(input, ctx) {
    ...
    const body = await client.request<{
      superagent_ids: string[];
      pagination: { total: number; limit: number; next_cursor?: string | null; has_more: boolean };
    }>(`/${encodeURIComponent(workspaceId)}/users/${encodeURIComponent(userId)}/superagents`, {
      query: compact({
        limit: p.limit,
        cursor: p.cursor,
    ...
    return {
      superagentIds: body.superagent_ids,
