...
/** Creating a new resource always mints a new row — none of these are safe to retry blindly. */
...
/** An update/delete/restore/tag-sync by id converges to the same state on retry. */
Deno.test("index: update/delete/restore/tag actions are marked idempotent", () => {
  for (
    const a of app.actions.filter((a) =>
      a.key.endsWith("-update") || a.key.endsWith("-delete") || a.key.endsWith("-restore") ||
      a.key.startsWith("contact-tags-")
    )
  ) {
...
Deno.test("index: no action hard-codes a host", async () => {
  for (const a of app.actions) {
    const src = await actionSource(a.key);
    assert(!/givebutter\.com/.test(src), `${a.key}: contains a Givebutter host literal`);
    assert(!/https?:\/\//.test(src), `${a.key}: contains an absolute URL`);
  }
});

/** No action reaches the SSO surface — see `auth/api-key.ts` for why it can't work anyway. */
Deno.test("index: no action or health check touches the /sso/v1 surface", async () => {
  for (const dir of ["actions", "auth", "health"]) {
    for await (const entry of Deno.readDir(new URL(`../${dir}`, import.meta.url))) {
      ...
      assert(
        !/\/sso\/v1/.test(src),
        `${dir}/${entry.name}: references the unreachable SSO surface`,
      );
    }
  }
});
...
Deno.test("index: the auth probe is /campaigns, never an /sso/v1 path", async () => {
  ...
  assert(
    !/PROBE_PATH\s*=\s*["'`]\/sso/.test(src),
    "the probe was pointed at the unreachable SSO surface",
  );
});
...
Deno.test("index: the manifest allows the API host and not the status host", async () => {
  ...
  assertEquals(manifest.w6w.id, "io.w6w.givebutter");
  assert(manifest.w6w.network.allow.includes("api.givebutter.com"));
  // The status host belongs to the health check's own allowlist, not the app's.
  assert(!manifest.w6w.network.allow.includes("givebutter.statuspage.io"));
...
Deno.test("index: the icon is the vendor's real mark", async () => {
  ...
  assert(bytes.length > 1000, "icon.png is suspiciously small for the vendor's real mark");
});
