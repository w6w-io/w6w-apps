...
Deno.test("service-account: is a custom method with email + private key + subject fields", () => {
  ...
  // `subject` is required — Domain-Wide Delegation without impersonation is
  // the single detail this whole flow hinges on.
  assertEquals(fields.find((f) => f.key === "subject")?.required, true);
});
...
Deno.test("service-account: test with missing subject reports a Domain-Wide Delegation failure", async () => {
  const { ctx, calls } = mockCtx();
  const result = await auth.test({
    credential: { email: "sa@proj.iam.gserviceaccount.com", privateKey: "pem" },
  }, ctx);
  assertEquals(result.ok, false);
  assert((result.message ?? "").includes("subject"));
  assertEquals(calls.length, 0);
});
...
Deno.test("service-account: exchange builds a JWT assertion with sub=subject and forwards the token", async () => {
  ...
  // Domain-Wide Delegation: without `sub`, the token is minted for the
  // service account's own identity and every Directory call is refused.
  assertEquals(claims.sub, "admin@example.com");
  assert(claims.scope.includes("admin.directory.user"));
  assert(claims.scope.includes("admin.directory.group"));
  assert(claims.scope.includes("admin.directory.orgunit"));
});
