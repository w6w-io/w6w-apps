...
Deno.test("service: unknown when the feed carries no Zoho Bookings component", () => {
  const result = service.check!(
...
/** "Zoho Books" and "Zoho Bookings" are separate, easily-confused components on the same feed. */
Deno.test("service: does not match the neighbouring Zoho Books component", () => {
  const result = service.check!(
    {
      feed: feedOf([
        "Zoho Books - Major Outage",
        "Zoho Mail - Major Outage",
        "Zoho Bookings - Operational",
      ]),
    },
    {} as never,
  );
...
Deno.test("service: maps Operational to ok", () => {
  const result = service.check!({ feed: feedOf(["Zoho Bookings - Operational"]) }, {} as never);
  assertEquals((result as { state: string }).state, "ok");
});

Deno.test("service: maps Major Outage to down", () => {
  const result = service.check!({ feed: feedOf(["Zoho Bookings - Major Outage"]) }, {} as never);
...
Deno.test("service: maps Degraded Performance and Partial Outage and Under Maintenance to degraded", () => {
  for (const status of ["Degraded Performance", "Partial Outage", "Under Maintenance"]) {
    const result = service.check!({ feed: feedOf([`Zoho Bookings - ${status}`]) }, {} as never);
...
Deno.test("service: an unrecognised status word is unknown, not guessed", () => {
  const result = service.check!({ feed: feedOf(["Zoho Bookings - Something New"]) }, {} as never);
  assertEquals((result as { state: string }).state, "unknown");
});
