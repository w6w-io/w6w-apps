/**
 * Is Zoho Bookings up? — Zoho's StatusIQ (Site24x7) status page.
 *
 * Verified live 2026-09-05: `https://us.zohostatus.com/rss` is a Site24x7
 * StatusIQ page that lists every Zoho product — Mail, CRM, Books, Bookings,
 * ~100 more — as one RSS item per component, titled `"{component} -
 * {status}"`. Fetched live: the entry is exactly `"Zoho Bookings -
 * Operational"`, distinct from the neighbouring `"Zoho Books -
 * Operational"` entry — the two products are easy to conflate by name but
 * are separate components on this feed. This is the same feed this pack's
 ...
 *     this hook only finds the "Zoho Bookings" component and reads its
...
/** Exact component name on the status page — "Zoho Bookings". */
const COMPONENT = "Zoho Bookings";

const service: HealthCheckDefinition = {
  key: "service",
  title: "Zoho Bookings platform status",
  description:
    'Reads the "Zoho Bookings" component off Zoho\'s StatusIQ RSS feed (us.zohostatus.com/rss). ' +
