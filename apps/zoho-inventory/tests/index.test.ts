...
/**
 * Creating a record starts a new resource on every call — retrying a dropped
 * response would create a duplicate, not converge on the same state.
 */
Deno.test("index: creates are not marked idempotent", () => {
  for (const key of ["contact-create", "item-create", "salesorder-create"]) {
    assertEquals(app.actions.find((a) => a.key === key)?.idempotent, false, key);
  }
});

/**
 * The converse: updates and deletes converge on the same end state no matter
 * how many times they run, so the runtime is free to retry them.
 */
...
/** Warehouses are read-only here — creating or deleting one is out of scope. */
Deno.test("index: locations are read-only", () => {
  const locationActions = app.actions.filter((a) => a.resource === "location");
  assertEquals(locationActions.map((a) => a.key), ["location-list"]);
  assertEquals(locationActions[0].type, "read");
});
...
/** Every action but the discovery call exposes the organization, optional. */
Deno.test("index: every action except organization-list takes an optional organizationId", () => {
  for (const a of app.actions) {
    if (a.key === "organization-list") {
      assertEquals(a.params ?? [], []);
      continue;
    }
    const param = (a.params ?? []).find((p) => p.key === "organizationId");
    assert(param, `${a.key}: no organizationId param`);
    assert(!param!.required, `${a.key}: organizationId must stay optional`);
  }
});
...
Deno.test("index: the manifest allows every region's API host and none of the accounts hosts", async () => {
  ...
  assertEquals(manifest.w6w.id, "io.w6w.zoho-inventory");
  for (const region of REGIONS) {
  ...
  // The eight API hosts, plus the unsigned status feed the `service` check
  // reads — and nothing else.
  assertEquals(manifest.w6w.network.allow.length, REGIONS.length + 1);
  assert(manifest.w6w.network.allow.includes("us.zohostatus.com"));
...
Deno.test("index: the icon is the placed vendor mark", async () => {
  ...
  assert(svg.includes("Zoho Inventory"), "missing the Zoho Inventory aria-label/title");
});
