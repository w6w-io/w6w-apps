...
/**
 * Items and Locations have no scope of their own — both are documented under
 * the general `settings` family, the same surprise Zoho Books has with Items.
 */
Deno.test("oauth2: the settings scope covers items and locations; no items/locations scope exists", () => {
  for (const method of oauth2Methods) {
    const scopes = method.oauth2?.scopes ?? [];
    assert(scopes.includes("ZohoInventory.settings.ALL"), "missing the settings scope");
    assert(
      !scopes.some((s) => /\.(items|locations)\./.test(s)),
      "claimed a scope Zoho does not offer",
    );
  }
});
...
Deno.test("oauth2-us: sign stamps the Zoho-oauthtoken header and nothing else", () => {
  ...
  assertEquals(signed.url, "https://www.zohoapis.com/inventory/v1/organizations");
...
Deno.test("oauth2-us: test passes when /organizations answers", async () => {
  ...
  assertEquals(new URL(calls[0].url).pathname, "/inventory/v1/organizations");
...
/**
 * Zoho Inventory's two documented codes are two different problems: no usable
...
/** The probe must classify by body, but must never echo the credential back. */
Deno.test("oauth2: no test-hook message ever contains the access token", async () => {
  for (
    const response of [
      { status: 401, body: { code: 14, message: "no auth" } },
      { status: 401, body: { code: 57, message: "not authorized" } },
