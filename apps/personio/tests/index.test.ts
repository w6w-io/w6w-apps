...
Deno.test("index: mutating actions are not marked idempotent", () => {
  for (const key of ["create-employee", "create-time-off", "upload-document"]) {
    assertEquals(
      app.actions.find((a) => a.key === key)?.idempotent,
      false,
      `${key}: should not be idempotent`,
    );
  }
});
...
/**
 * The one Personio host lives in `lib/client.ts` and nowhere else. An action that
 * hard-coded the host could be pointed somewhere the manifest never allowlisted.
 */
Deno.test("index: no action hard-codes the Personio host", async () => {
  for (const a of app.actions) {
    const src = await actionSource(a.key);
    assert(!/api\.personio\.de/.test(src), `${a.key}: contains a Personio host literal`);
...
Deno.test("index: the manifest allows the Personio API host and not the status host", async () => {
  const manifest = JSON.parse(
    await Deno.readTextFile(new URL("../package.json", import.meta.url)),
  ) as { w6w: { id: string; network: { allow: string[] }; appearance: { icon: { svg: string } } } };
  assertEquals(manifest.w6w.id, "io.w6w.personio");
  assert(manifest.w6w.network.allow.includes("api.personio.de"));
  assert(!manifest.w6w.network.allow.includes("status.personio.de"));
  assertEquals(manifest.w6w.appearance.icon.svg, "./assets/icon.svg");
});

Deno.test("index: the icon is the simple-icons Personio glyph, downloaded verbatim", async () => {
  const bytes = await Deno.readFile(new URL("../assets/icon.svg", import.meta.url));
  assertEquals(bytes.byteLength, 1115);
  const text = new TextDecoder().decode(bytes);
  assert(text.includes("<title>Personio</title>"));
});
