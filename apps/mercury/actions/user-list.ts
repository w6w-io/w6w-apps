...
/** `GET /users` — "Get all users". `operationId: getUsers` (unnamed in the summary). */
interface Input {
  limit?: number;
  order?: "asc" | "desc";
  startAfter?: string;
...
const userList: ActionDefinition<Input> = {
  ...
  resource: "user",
  title: "List Users",
  description: "List every user on the organization.",
  ...
  async execute(input, ctx) {
    const body = await new MercuryClient(ctx).json<UsersResponse>("/users", {
      query: {
        limit: input.limit,
        order: input.order,
