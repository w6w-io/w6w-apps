...
Deno.test("service: the reason names the surfaces that were ruled out, with what they answered", () => {
  ...
  // A 200 that is a client-rendered shell is not an endpoint.
...
Deno.test("request-rate: declared absent, because SharePoint retracted the RateLimit headers", () => {
...
Deno.test("quota: is a real probe against the drive's own storage facet", () => {
...
Deno.test("quota: reads the vendor's own state rather than re-deriving one", async () => {
  const { ctx } = mockCtx([{
    body: {
      driveType: "business",
      quota: { total: 1000, used: 200, remaining: 800, deleted: 5, state: "normal" },
    },
  }]);
  const out = await quota.check!({} as never, ctx);
  assertEquals(out.state, "ok");
  assert(out.message?.includes("business drive"), out.message);
...
Deno.test("quota: falls back to the byte counts when a drive reports no state", async () => {
  // SharePoint libraries on an unlimited plan do this.
...
Deno.test("quota: a full drive with no state is down", async () => {
...
Deno.test("quota: a drive with no quota facet is unknown, not a verdict", async () => {
...
Deno.test("quota: reports no RateLimit-based reading, even when headers are present", async () => {
  // SharePoint says it does not send these; a check must not depend on them.
  const { ctx } = mockCtx([{
    body: { quota: { total: 10, remaining: 9, state: "normal" } },
    headers: {
  ...
  const out = await quota.check!({} as never, ctx);
  assertEquals(out.quota?.map((q) => q.id), ["storage"]);
});
