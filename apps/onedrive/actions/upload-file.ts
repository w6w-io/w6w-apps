...
import { driveIdParam, itemOutput, itemParams } from "../lib/params.ts";
...
/**
 * `PUT /me/drive/items/{parent-id}:/{filename}:/content` — or
 * `PUT /me/drive/items/{item-id}/content` to replace a file in place.
 *
 * https://learn.microsoft.com/en-us/graph/api/driveitem-put-content
 *
 * The simple upload. Two limits, one of them Microsoft's and one of them ours:
 *
 *  - **Microsoft's:** "This method only supports files up to 250 MB in size."
 *    (The 4 MB figure that circulates is from an older revision of this page.)
 ...
 *    upload is deliberately not offered. Responses cross back as bytes, so the
 *    asymmetry is real and only affects uploads.
 *
 * Addressing follows the endpoint's three documented forms: give a **File name**
 * and the addressed item is the *parent folder*; leave it empty and the
 * addressed item is the file being replaced.
 *
 * Least privileged delegated permission: `Files.ReadWrite`. Returns the created
 * or updated driveItem (`201` for a new file, `200` for a replacement).
 */
