...
import { DOCS_API } from "../lib/client.ts";
 ...
 * enable the Google Docs API + Google Drive API, then store the
 ...
 *
 * Google specifics:
 *   - Two scopes: `documents` (read+write document content) and `drive.file`
 *     (create + access files this app touches — needed to create the Docs
 *     resource via Drive's `files` endpoint).
...
const oauth2: AuthDefinition = {
  ...
  description:
    "Public OAuth flow. Requires a Google Cloud OAuth 2.0 client (client_id / client_secret / redirect_uri) configured on this w6w installation, with the Google Docs and Drive APIs enabled.",
  ...
  async test({ credential }, ctx) {
    ...
    // No trivial GET-me endpoint for Docs — validate the token itself with
