...
/**
 * Creating a calendar or an event starts a new resource on every call — retrying a dropped
 * response would create a duplicate, not converge on the same state.
 */
Deno.test("index: creates are not marked idempotent", () => {
  for (const key of ["calendar-create", "event-create"]) {
    assertEquals(app.actions.find((a) => a.key === key)?.idempotent, false, key);
  }
});

/**
 * The converse: updates and deletes converge on the same end state no matter how many times they
 * run (an update always sends the full desired state; a delete of an already-deleted record is a
 * no-op from the caller's point of view), so the runtime is free to retry them.
 */
Deno.test("index: updates and deletes are marked idempotent", () => {
  for (
    const key of ["calendar-update", "calendar-delete", "event-update", "event-delete"]
  ) {
 ...
 *
 * Without this the checks are simultaneously too weak and too strong: a doc comment explaining
 * *why* an action never touches the credential trips the assertion, while a reviewer's natural
 * fix — deleting the explanation — would leave a real violation just as invisible.
 */
...
/**
 * Every API host lives in `lib/regions.ts` / `lib/client.ts` and nowhere else. An action that
 * hard-coded a `calendar.zoho.<tld>` host — or accepted one as a param — could be pointed
...
Deno.test("index: no action hard-codes a host or an absolute URL", async () => {
  for (const a of app.actions) {
    ...
    assert(
      !/zoho\.com|zoho\.eu|zoho\.in|zohocloud|zoho\.jp|zoho\.sa/.test(src),
...
Deno.test("index: the manifest allows every region's API host and none of the accounts hosts", async () => {
  ...
  assertEquals(manifest.w6w.id, "io.w6w.zoho-calendar");
  for (const region of REGIONS) {
...
Deno.test("index: the icon is the placed vendor mark", async () => {
