 ...
 *
 * Notes:
 *   - Sheets accessed by a service account must be explicitly shared with the
...
interface ServiceAccountCredential {
  email: string;
  privateKey: string;
  /** Optional space-separated OAuth scopes. Defaults to sheets + drive.file. */
  scopes?: string;
...
const serviceAccount: AuthDefinition = {
  ...
  description:
    "Server-to-server auth using a Google service account. Paste the service account's email and PEM private key (from a downloaded JSON key). Remember to share the target spreadsheet with the service account's email.",
