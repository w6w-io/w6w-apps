 ...
 * action doc comments here reference "credential" while explaining why they
 * never touch one.
 */
...
Deno.test("index: no action reads a credential — signing is the auth hook's job", async () => {
  for (const a of app.actions) {
    ...
    assert(!/access_token/i.test(src), `${a.key}: sets the access token header itself`);
  }
});
...
Deno.test("index: the sole health check is informational and declares no execution hook", () => {
